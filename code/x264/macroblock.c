...
{
    fp = fopen("/home/jun/Desktop/dct","rwa+");
     int32_t temp = -98;
     static int count = 0; 
    // printf("the temp interger is %d\n", temp);
     char temps[33];
     char s[12] = "a good news";
   //  fp= open("/home/jun/Desktop/dct", O_CREAT | O_WRONLY | O_APPEND, S_IRUSR | S_IWUSR);
    if(fp == NULL)
     printf("fail to open the dct file\n");
    ...
    printf("pdst size is %d, the value p is %d\n", sizeof(p_dst)/sizeof(p_dst[0]),p);
