...
{
    x264_emms();
    float qp = h->rc->qpm; //this value will be overwritten
    ...
    // Test Modification of the QP value
    // We will try a constant QP value
    //qp = 45; //we overwrite the qp value with 0
    /* if( saliencyScore < 0.500) //if below our threshold saliencyScore, do not allocate much bitrate
    {
        qp = 40; //40 qp is basically how much the average compression should be, max compression would be 51
    }
    else
    {
        qp = 25;
    ...
    /*
    double multiplierIndex = 0.03571; //divides the saliencyScore indices for range comparison, this is 1/28, there are 28 fractions between 0(no saliency!) and 1(lotsa saliency)
    for( int x=0; x < 28; x++)
    {
        double saliencyScoreRange = ((double)x)*multiplierIndex;
        if( saliencyScore <= saliencyScoreRange) //this allocates qp dynamically based on the calculated saliency score of the block, it will keep looping through the for loop 28 times until it has found the correct qp value for the saliency score
        {
            qp = (int)(51 - 3*x); //does not go below 23 qp or higher than 51 qp
	    if (qp < 23)
            {qp = 23;}    
            break;
        }
    ...
    else if(saliencyScore <= 0.1 && saliencyScore > 0)
    {
        qp = 45;
    }

    else if(saliencyScore <= 0.2 && saliencyScore > 0.1)
    {
        qp = 35;
    }

    else if(saliencyScore <= 0.3 && saliencyScore > 0.2)
    {
        qp = 30;
    }
   
    else if(saliencyScore <= 0.4 && saliencyScore > 0.3)
    {
        qp = 29;
    }

    else if(saliencyScore <= 0.5 && saliencyScore > 0.4)
    {
        qp = 28;
    }

    else if(saliencyScore <= 0.6 && saliencyScore > 0.5)
    {
        qp = 27;
    }

    else if(saliencyScore <= 0.7 && saliencyScore > 0.6)
    {
        qp = 26;
    }

    else if(saliencyScore <= 0.8 && saliencyScore > 0.7)
    {
        qp = 25;
    }

    else if(saliencyScore <= 0.9 && saliencyScore > 0.8)
    {
        qp = 24;
    }

    else if(saliencyScore <= 1 && saliencyScore > 0.9)
    {
        qp = 23;
    }
